...
if [ -z "$TARGET" ]; then
  echo "${RED}Seteaza TARGET=<IP-ul VM-ului backend>, ex: TARGET=192.168.64.4 ./attack-sim-mac.sh${RESET}" >&2
  exit 1
fi

command -v ssh >/dev/null 2>&1 || { echo "${RED}Lipseste 'ssh'.${RESET}" >&2; exit 1; }
command -v nc  >/dev/null 2>&1 || { echo "${RED}Lipseste 'nc'.${RESET}" >&2; exit 1; }
if [ "$DO_BRUTE" = "1" ] && ! command -v sshpass >/dev/null 2>&1; then
  echo "${RED}Lipseste 'sshpass'. Ruleaza: brew install hudochenkov/sshpass/sshpass${RESET}" >&2
  exit 1
fi

echo "${BOLD}Simulator atac (de pe Mac) -> ${TARGET}${RESET}"
...
echo "  ${YELLOW}ATENTIE: backend-ul va vedea atacul de pe IP-ul Mac-ului (gateway). Dupa block,${RESET}"
echo "  ${YELLOW}VM-ul pierde accesul spre Mac/internet. Deblochezi din UI pe VM -> Firewall.${RESET}"
echo
...
knock_once() { nc -z -w 2 "$TARGET" "$1" >/dev/null 2>&1; }

port_open() {
  if nc -z -w 3 "$TARGET" "$KNOCK_PROTECTED_PORT" >/dev/null 2>&1; then echo yes; else echo no; fi
}
...
if [ "$DO_KNOCK" = "1" ]; then
  ...
  echo "  ${YELLOW}>> Uita-te in UI: ar trebui o alerta de PORT KNOCK pentru IP-ul Mac-ului.${RESET}"
  echo
fi
...
if [ "$DO_BRUTE" = "1" ]; then
  ...
  echo
  echo "  ${YELLOW}>> Uita-te in UI (in ~15s): alerta BLOCKED. Dupa block, Mac-ul nu mai ajunge la ${TARGET}.${RESET}"
  echo "  ${DIM}deblochezi din UI pe VM -> Firewall, ca sa revina accesul si sa poti repeta.${RESET}"
  echo
fi
